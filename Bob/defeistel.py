...
def defeistel_block(chunk, key_i):  # funzione che applica defeistel ad un blocco
    ...
    chunk.pos = 0
    key_i = key_i * 16  # ripeto la chiave 8 volte (128 bit di chiave per fare l'exor bit a bit con il chunk)

    # Liplus1 (256 bit) lo divido in 2 parti perchè l'MD5 processa solo 128 bit per volta

    # Leggo i primi 128 bit di Li+1
    Liplus1 = BitStream(bin=chunk.read('bin:256'))
    Liplus1_xor_key_i = BitStream(Liplus1 ^ key_i)
    Liplus1_md5 = hashlib.sha256()
    Liplus1_md5.update(Liplus1_xor_key_i.read('bin:256').encode('utf-8'))
    ...
    # Liplus2_md5.update(Liplus2_xor_key_i.read('bin:128').encode('utf-8'))
